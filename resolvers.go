...
// OptionUnmarshaler is an interface used to convert a string value extracted
...
func getResolver(fType reflect.Type, name string) OptionUnmarshaler {
	if name == NameTag {
		return &nameResolver{
			resolver: getResolver(fType, ""),
		}
	}
	if fType.Implements(reflect.TypeOf((*OptionUnmarshaler)(nil)).Elem()) {
		return reflect.New(fType).Interface().(OptionUnmarshaler)
	}
