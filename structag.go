...
// FieldTag is the parsed struct tag value for the struct field with the
...
// Option is the definition of an option for a defined struct tag type. An example being how
// encoding/json has "name", "omitempty", and "string" as options.
type Option struct {
	Name       string
	Required   bool
	FieldIndex int
	Resolver   OptionUnmarshaler
}

// Cache is a cache for parsed struct tags. It is used to parse a struct's tag defined
...
type Cache[T any] struct {
	...
	structTagMap map[string]Option
	hasName      bool
	requiredTags []string
}

// NewCache initializes a Cache for type T.
func NewCache[T any](tagName string) (*Cache[T], error) {
	...
	hasName := false
	structTagMap := make(map[string]Option)
	...
	for i := 0; i < defType.NumField(); i++ {
		...
		if structTag.Name != EmptyTag && structTag.Name != SkipTag {
			...
			case reflect.Slice, reflect.Array, reflect.Chan, reflect.Func, reflect.Interface, reflect.Invalid, reflect.Map, reflect.UnsafePointer:
				...
				if !field.Type.Implements(reflect.TypeOf((*OptionUnmarshaler)(nil)).Elem()) {
...
func (t *Cache[T]) Add(rType reflect.Type) error {
	kind := rType.Kind()
...
func (t *Cache[T]) Get(rType reflect.Type) ([]FieldTag[T], bool) {
...
// otherwise.
func (t *Cache[T]) GetOrAdd(rType reflect.Type) ([]FieldTag[T], error) {
	tags, ok := t.typeToTags[rType]
	if !ok {
		err := t.Add(rType)
...
// Parse parses the struct tags for a given type and converts them to type T.
func Parse[T any](tagName string, rType reflect.Type) ([]FieldTag[T], error) {
	cache, err := NewCache[T](tagName)
	if err != nil {
		return nil, err
	}
