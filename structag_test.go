...
import (
	...
	"github.com/bingoohuang/structag"
)
...
func TestNewTagCache(t *testing.T) {
	type NewTest struct {
		S string `structag:"name"`
	}
	cache, err := structag.NewCache[NewTest]("test")
...
func TestNewTagCacheInvalid(t *testing.T) {
	cache, err := structag.NewCache[string]("test")
	if cache != nil || err == nil {
		t.Error("TestNewTagCacheInvalid: failed struct validation")
	}
	type Bad struct {
		F  int `structag:"name"`
		F2 int `structag:"name"`
	}
	badCache, err := structag.NewCache[Bad]("test")
...
func TestQuotedTags(t *testing.T) {
	...
	cache, _ := structag.NewCache[TestQuotedTag]("test")
...
func TestSpecialTags(t *testing.T) {
	type TestSpecialTag struct {
		...
		Required string  `structag:"r,required"`
		Pointer  *string `structag:"p"`
	}
	type TestSpecialStructValid struct {
		Valid int `test:"name,r='r',p=p"`
	}
	cache, _ := structag.NewCache[TestSpecialTag]("test")
...
func TestTypeConversion(t *testing.T) {
	type TestTagTypes struct {
		String     string        `structag:"s"`
		Bool       bool          `structag:"b"`
		Int        int           `structag:"i"`
		Int8       int8          `structag:"i8"`
		Int16      int16         `structag:"i16"`
		Int32      int32         `structag:"i32"`
		Int64      int64         `structag:"i64"`
		Uint       uint          `structag:"u"`
		Uint8      uint8         `structag:"u8"`
		Uint16     uint16        `structag:"u16"`
		Uint32     uint32        `structag:"u32"`
		Uint64     uint64        `structag:"u64"`
		Float32    float32       `structag:"f32"`
		Float64    float64       `structag:"f64"`
		Complex64  complex64     `structag:"c64"`
		Complex128 complex128    `structag:"c128"`
		CustomType CustomType    `structag:"ct"`
		StringList []string      `structag:"sa"`
		IntList    []int         `structag:"ia"`
		Duration   time.Duration `structag:"d"`
	}
	cache, _ := structag.NewCache[TestTagTypes]("test")
